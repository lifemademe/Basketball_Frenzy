...
SOURCE_GLB = MODEL_DIR / "Badtarget.glb"
OUTPUT_GLB = MODEL_DIR / "Badtarget_runtime_v2.glb"
LEGACY_GLB = MODEL_DIR / "Badtarget_runtime.glb"
...
def main():
    ...
    optimized_count = sum(len(obj.data.polygons) for obj in meshes)
    if LEGACY_GLB.exists() and LEGACY_GLB != OUTPUT_GLB:
        LEGACY_GLB.unlink()
    print(f"Optimized Badtarget: {polygon_count} -> {optimized_count} polygons")
